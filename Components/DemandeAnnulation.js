...
class DemandeAnnulation extends React.Component {
    ...
    NetInfo.fetch().then(state => {
      ...
      AsyncStorage.getItem('token').then(value => {
        if (state.isConnected) {
          ...
          fetch(this.url + '/api/client/Liste_demande_annulation', {
            method: 'GET',
            headers: {
  ...
  handleSearch = (text) => {
    ...
    const orderData = _.filter(this.state.fullData, item => {
      if (moment(item.created_at).format('LLLL').toLowerCase().includes(formattedQuery) || this._formatStatus(item.statut_dem).toLowerCase().includes(formattedQuery) || this.format_id(item.id).toLowerCase().includes(formattedQuery) || this.format_code(item.reservation_id, item.date_res).toLowerCase().includes(formattedQuery)) {
        return true
      }
      return false
    })
        ...
        <View style={styles.container}>
          <Container style={styles.searchContainer}>
            ...
            <View style={styles.message}>
              <Text style={styles.messageText}>Aucune demande d'annulation</Text>
            </View>
          </Container>
        </View>
      )
                  ...
                  renderItem={({ item }) => (
                    <View style={[styles.listContainer, { backgroundColor: item.statut_dem === 1 ? '#009B77' : item.statut_dem === 2 ? 'red' : '#33A0D6' }]}>
                      ...
                      <View style={styles.title}>
                        ...
                        <Text style={styles.titleText}>{this.format_code(item.reservation_id, item.date_res)}</Text>
                      </View>
                    </View>
                  )}
                />
