...
class VuePrestationsPassees extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      orderData: undefined,
    ...
    NetInfo.fetch().then(state => {
      ...
      AsyncStorage.getItem('token').then(value => {
        if (state.isConnected) {
          ...
          fetch(this.url + '/api/client/Liste_pres_passee', {
            method: 'GET',
            headers: {
        ...
        <View style={styles.container}>
          <Container style={styles.searchContainer}>
            ...
            <View style={styles.message}>
              <Text style={styles.messageText}>Aucune prestation passée</Text>
            </View>
          </Container>
        </View>
      )
            ...
            {isLoading ?
              this._displayLoading() : (
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <View style={[styles.listContainer, { backgroundColor: item.statut_pres === 3 ? '#009B77' : '#11539F' }]}>
                      <View style={styles.dateContainer}>
                        ...
                        <View style={styles.badgeContainer}>
                          ...
                          <View style={styles.status}>
                            <Text style={[styles.statusText, { color: item.statut_pres === 3 ? '#009B77' : '#11539F' }]}>{this._formatStatus(item.statut_pres)}</Text>
                          </View>
...
export default VuePrestationsPassees